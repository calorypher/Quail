...
- The existing `SearchPerformanceTrace` remains the timing source. The only production addition is an opt-in private scenario input that drives the existing QueryBox path and labels trace events with a non-sensitive scenario id. Normal launch, search, ranking, storage, source composition, and UI behavior are unchanged when that option is absent.
...
The committed file contains only replaceable example query strings. M16-B must use an ignored local copy with stable, non-sensitive queries known to match the physical host's intended real index. Results and traces intentionally retain only scenario id, query length, timing metadata, and index scale.

## Validation evidence

| Check | Result |
| --- | --- |
| Focused configuration, scenario-parser, and trace tests | PASS — 11 tests, Release configuration |
...
It should confirm that `results.json` contains one sample for each requested id and that each sample has `inputToFirstTextMilliseconds`, `typingBurstToFirstTextMilliseconds`, and a corresponding JSONL trace. This is a harness validation only, not M16-B.
