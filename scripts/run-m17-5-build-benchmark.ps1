...
if (-not $principal.IsInRole([Security.Principal.WindowsBuiltInRole]::Administrator)) {
    throw 'M17.5 requires an elevated PowerShell session for the physical C: NTFS benchmark.'
}

if ([string]::IsNullOrWhiteSpace($OutputDirectory)) {
    $OutputDirectory = Join-Path $repositoryRoot (Join-Path 'artifacts\m17.5' (Get-Date -Format 'yyyyMMdd-HHmmss'))
}
